...
- Maven 多模块
- MySQL + MyBatis-Plus：商品、评价、优惠券、购物车、订单、秒杀活动、秒杀结果、可靠消息记录与消费幂等
- Redis：商品缓存、购物车缓存、秒杀库存预扣
...
- OpenFeign：订单服务调用商品、购物车内部接口，商品服务调用评价、优惠券内部接口
...
- `mall-product`：商品详情、分类树、库存扣减、Seata TCC 库存预留、CompletableFuture 异步聚合评价和优惠券、TTL 线程池上下文传播、Canal 监听 Binlog 后自动缓存失效
- `mall-review`：商品评价摘要查询
- `mall-coupon`：商品可领取优惠券查询
- `mall-cart`：购物车增删改查、勾选状态、用户隔离，MySQL 持久化并使用 Redis 缓存
- `mall-order`：订单确认、创建、支付、取消、Seata 全局事务、OpenFeign 调用商品/购物车、RabbitMQ 延迟关单、库存释放
- `mall-seckill`：秒杀活动、Sentinel 限流、Redisson 防重复提交锁、Redis Lua 扣库存、一人一单、RabbitMQ 异步创建订单
...
注意：迁移脚本会重建 `mq_message`、`consume_record`、`seckill_activity`、`seckill_sku`、`seckill_result`，用于本地演示库升级。导入中文 SQL 时不要用 `Get-Content ... | docker compose exec ...` 管道方式，避免 PowerShell 编码把中文写坏。
...
- `mall-seckill` 使用 `@RabbitListener` 消费秒杀结果消息，把 `requestId` 更新为 `SUCCESS/FAILED`。
- 消费成功后手动 `basicAck`。
...
- 已完成 MyBatis-Plus + MySQL 接入：`mall-product` 查询商品和库存，`mall-review` 持久化评价摘要，`mall-coupon` 持久化商品优惠券，`mall-cart` 持久化购物车，`mall-order` 持久化订单、订单明细和秒杀订单，`mall-seckill` 持久化秒杀活动、秒杀库存和查询结果，`mall-message` 持久化可靠消息和消费记录。
...
- 已接入 Redisson：秒杀提交按 `activityId + skuId + userId` 加短租约分布式锁，控制同一用户重复提交；库存扣减仍由 Redis Lua 脚本保证原子性。
...
订单服务通过 MyBatis-Plus 持久化 `order_info`、`order_item`、`seckill_order`。订单创建后发送 RabbitMQ 延迟关单消息，消息先写入 `mq_message`，再进入 `mall.order.close.delay.queue`，到期后通过死信转发到 `mall.order.close.queue`，订单服务消费后只关闭 `CREATED` 状态订单并释放库存。

秒杀链路使用 Sentinel 对 `seckill-submit` 资源做 QPS 限流，Redisson 对同一用户同一活动 SKU 的重复提交加短租约锁，再通过 Redis Lua 脚本完成库存扣减和一人一单校验；扣减成功后发送 RabbitMQ 下单消息，由订单服务异步创建秒杀订单；订单创建成功后再发送秒杀结果消息，秒杀服务消费后把查询结果从 `PROCESSING` 更新为 `SUCCESS` 并写入 `seckill_result.order_sn`。
